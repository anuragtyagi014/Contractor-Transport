...
function business_club_customize_register($wp_customize)
{
	...
	$wp_customize->add_section(
		new Business_Club_Upsell_Section(
			$wp_customize,
			'custom_theme_upsell',
			array(
				'title'    => esc_html__('Business Club Pro', 'business-club'),
				'pro_text' => esc_html__('Buy Pro', 'business-club'),
				'pro_url'  => 'https://axlethemes.com/wordpress-themes/business-club-pro/',
				'priority' => 1,
			)
		)
	);
...
function business_club_customizer_control_scripts()
{
	...
	wp_enqueue_script('business-club-customize-controls', get_template_directory_uri() . '/js/customize-controls' . $min . '.js', array('jquery', 'customize-controls'), '2.0.2');
	wp_enqueue_style('business-club-customize-controls', get_template_directory_uri() . '/css/customize-controls' . $min . '.css', array(), '2.0.2');
}
